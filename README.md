...
| `MEOW_NICKNAME`     | ❌ 否 | MeoW 推送的用户昵称（从 [API 文档](https://www.chuckfang.com/MeoW/api_doc.html) 获取） |
| `PUSH_LEVEL`        | ❌ 否 | 推送级别：`all` (默认，每次均推送) 或 `fail_only` (仅有账号签到失败时推送) |
| `GLADOS_EXCHANGE_PLAN` | ❌ 否 | 积分自动兑换档位，可选 `100` / `200` / `500`（分别兑换 10/30/100 天会员）。签到时总积分达到所选档位即自动兑换，不配置则默认不兑换。 |
...
| `MEOW_NICKNAME`         | MeoW 推送昵称（见下方）  | ❌ 否 |
| `GLADOS_EXCHANGE_PLAN`  | `100` / `200` / `500` 三选一（积分达到即自动兑换） | ❌ 否 |

---

### 第四步：配置 MeoW 推送（可选）📱

如果你希望签到后收到通知，请配置 MeoW 推送：

1. 访问 [MeoW API 文档](https://www.chuckfang.com/MeoW/api_doc.html) 了解推送服务
2. 获取你的**用户昵称**（用于接收推送）
3. 将昵称添加到 GitHub Secrets，Name 填 `MEOW_NICKNAME`
